...
async def get_current_user(token:HTTPAuthorizationCredentials = Depends(bearer),connection:AsyncSession = Depends(get_session)):
    ...
    user = await connection.scalar(select(User).
                                   options(selectinload(User.points), selectinload(User.basket), selectinload(User.products)).
...
@app.put("/update_me", response_model=UserMe)
