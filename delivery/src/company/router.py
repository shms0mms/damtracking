...
async def check_addres(address:str, me:User = Depends(get_current_company),session:AsyncSession = Depends(get_session)):
    
    

    point = Points(address = address, company_id = me.id)
...
async def add_product(data:ProductCreate,me:User = Depends(get_current_company),session:AsyncSession = Depends(get_session)):
    ...
    product = Products(title = data.title, desc = data.desc  )
